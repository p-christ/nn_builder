...
def test_linear_layers():
    ...
    for input_dim, output_dim, hidden_units in zip( range(5, 8), range(9, 12), [[2, 9, 2], [3, 5, 6], [9, 12, 2]]):
        ...
        nn_instance = NN(input_dim=input_dim, layers=hidden_units,
                         hidden_activations="relu",
                         output_activation="relu", initialiser="xavier")
        assert "input" in nn_instance.layers[0].__dict__["_name"]
        for layer_ix in range(1, len(hidden_units) + 1):
            print(nn_instance.layers[layer_ix])
            assert type(nn_instance.layers[layer_ix]) == tf.keras.layers.Dense
            assert nn_instance.layers[layer_ix].units == hidden_units[layer_ix - 1]
...
def solves_simple_problem(X, y, nn_instance):
    ...
    for ix in range(800):
        out = nn_instance.forward(X)
        loss = torch.reduce_sum((out.squeeze() - y) ** 2) / N
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    return loss < 0.1
