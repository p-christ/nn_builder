...
def test_output_heads_error_catching():
    ...
    for output_dim in output_dims_that_should_break:
        with pytest.raises(AssertionError):
            NN(input_dim=2, layers=[4, 7, 9, output_dim], hidden_activations="relu",
    ...
    for output_activation in output_activations_that_should_break:
        with pytest.raises(AssertionError):
            NN(input_dim=2, layers=[4, 7, 9, [4, 6, 1]], hidden_activations="relu",
...
def test_output_head_layers():
    ...
    for output_dim in [[3, 9], [4, 20], [1, 1]]:
        nn_instance = NN(input_dim=2, layers=[4, 7, 9, output_dim], hidden_activations="relu",
...
def test_output_head_activations_work():
    ...
    nn_instance = NN(input_dim=2, layers=[4, 7, 9, [5, 10, 3]], hidden_activations="relu",
