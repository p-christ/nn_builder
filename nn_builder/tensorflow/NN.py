...
def NN(input_dim: int, layers: list, output_activation=None,
    ...
    nn_creator = NN_Creator(input_dim, layers, output_activation,
                 hidden_activations, dropout, initialiser, batch_norm,
                 columns_of_data_to_be_embedded, embedding_dimensions, y_range,
                 random_seed, print_model_summary)

    model = nn_creator.create_model()
    return model


class NN_Creator(TensorFlow_Base_Network):
    ...
    def __init__(self, input_dim: int, layers: list, output_activation=None,
        ...
        TensorFlow_Base_Network.__init__(self, input_dim, layers, output_activation,
    ...
    def create_hidden_layers(self):
        ...
        linear_layers = []
        for layer_ix, hidden_unit in enumerate(self.layers[:-1]):
            activation = self.get_activation(self.hidden_activations, layer_ix)
            linear_layers.extend([tf.keras.layers.Dense(hidden_unit, activation=activation,
    ...
    def incorporate_embeddings(self, x):
        ...
        all_embedded_data = Concatenate(axis=1)(tuple(all_embedded_data))
        x = Concatenate(axis=1)((x[:, [col for col in range(x.shape[1]) if col not in self.columns_of_data_to_be_embedded]].float(), all_embedded_data))
        return x
